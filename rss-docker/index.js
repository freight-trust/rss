...
if (process.env.FLUENTD_HOST) {
  var tags = (process.env.FLUENTD_TAGS
    ? process.env.FLUENTD_TAGS.split(",")
    : []
  ).reduce((allTags, tag) => {
    var pair = tag.split(":");
  ...
  log4js.addAppender(
    require("fluent-logger").support.log4jsAppender("docker-hub-rss", {
...
var tags = (process.env.STATSD_TAGS
  ? process.env.STATSD_TAGS.split(",")
  : []
).reduce((allTags, tag) => {
  var pair = tag.split(":");
  allTags[pair[0].trim()] = pair.length === 1 ? true : pair[1].trim();
...
function getTagsRecursive(username, repository, page, tags) {
  return dockerHubAPI.tags(username, repository, { page }).then((tagsPage) => {
